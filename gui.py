...
def update_grid(number, pos):
    display_val = int(number) if number != 0 else ""
    row = pos//9
    col = pos-row*9
    cells[row][col].configure(text=display_val)
    window.update()

puzzle = fetch_puzzle("Easy")
def select_difficulty(level: str):
    if level == "Master": level = "evil"
    k = fetch_puzzle(level)
    puzzle[:] = k[:]
    for index, num in enumerate(puzzle):
        update_grid(num, index)
...
for level in levels:
    btn = ctk.CTkButton(
        ...
        text=level,
        width=80,
        command=lambda l=level: select_difficulty(l)
    )
...
for i in range(9):
    ...
    for j in range(9):
        ...
        cell = ctk.CTkLabel(
            grid_frame,
            text=int(puzzle[index]) if puzzle[index]!=0 else "",
            width=45, height=45,
            fg_color="#434343",
            corner_radius=2,
        )
